$(document).ready(function(){
  var HTTP_BASE_PATH = 'https://app.shopping-cart-migration.com/';
  var HTTP_API_PATH = HTTP_BASE_PATH + 'capi/';
    ...
    $.ajax({
      ...
      success: function(data){
        if (data.error == false){
          ...
          errorMessage(data.data,'error');
          submitCart2cartRemoteForm.removeClass('loading');
        }
      },
  ...
  function saveFtpInfo(host,user,pass,dir){
    ...
    $.ajax({
      ...
      success: function(response){
        ...
        console.log(data.messageType + ': ' + data.messages);
        $("#ajaxloader").hide();
        if (data.messageType != 'error') {
          $("#setup_target_link").click();
        }
        console.log('FtpInfo saved');
        submitCart2cartRemoteForm.removeClass('loading');
      },
      error: function(response){
        console.log('Failed to save FtpInfo');
        submitCart2cartRemoteForm.removeClass('loading');
      }
    });
  }
  ...
  submitCart2cartRemoteForm.on("click",function(){
    ...
    if (!hasError){
      $(".c2cReqired").hide();
      $("#ajaxloader").show();
      submitCart2cartRemoteForm.addClass('loading');
      getToken();
    }
  });
  ...
  $(function (){
    ...
    tabNavigation.click(function () {
      tabContainers.hide();
      tabContainers.filter(this.hash + '_content').show();
